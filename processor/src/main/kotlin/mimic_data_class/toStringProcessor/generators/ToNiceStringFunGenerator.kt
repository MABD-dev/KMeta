...
internal class ToNiceStringFunGenerator (
    ...
    private fun List<KSValueParameter>.toNameValuePair(logger: KSPLogger): List<Pair<String, String>> {
        return this.mapNotNull { param ->
            ...
            val paramTypeAnnotations = param.type.resolve().declaration.annotations

            val hasToNiceString = paramTypeAnnotations.distinct().firstOrNull {
                val qualifiedName = it.annotationType.resolve().declaration.qualifiedName?.asString() ?: false
                qualifiedName == ToNiceString::class.java.name
            } != null

            val value = if (hasToNiceString) {
                "\${${name}.toNiceString()}"
            } else {
                "\$${name}"
            }
